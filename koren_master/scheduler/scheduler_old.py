    ...
    try:
        for n in v1_client.list_node().items:
            if n.metadata.labels.get("noCustomScheduler") == "yes":
                logger.info("Skipping Node {n.metadata.name} since it has noCustomScheduler label")
                continue
            if filtered:
...
def _get_schedulable_node(v1_client,end_device):
    ...
    # end_device = '116.89.189.24'
    network_status = requests.get('http://116.89.189.24:4000/api/rtt/'+end_device)
        ...
        try:
            ...
            try:
                ...
                for event in watcher.stream(V1_CLIENT.list_pod_for_all_namespaces, label_selector=SCHEDULE_STRATEGY, timeout_seconds=20):
                    logger.info("Event: {event['type']} {event['object'].kind}, {event['object'].metadata.namespace}, {event['object'].metadata.name}, {event['object'].status.phase}")
                    if event["object"].status.phase == "Pending":
                        try:
                            logger.info('{event["object"].metadata.name} needs scheduling...')
                            ...
                            node_name = _get_schedulable_node(V1_CLIENT,pod_enddevice) # get schedulable node
                            if node_name:
                                ...
                                logger.error("Found no valid node to schedule {pod_name} in {pod_namespace}")
                        except ApiException as e:
                            logger.error(json_loads(e.body)["message"])
