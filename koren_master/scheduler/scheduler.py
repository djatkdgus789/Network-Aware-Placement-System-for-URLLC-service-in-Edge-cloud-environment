...
def _get_schedulable_node(v1_client,end_device):
    ...
    # end_device = '10.20.12.83'
    network_status = requests.get('http://10.20.12.83:4000/api/rtt/'+end_device)
    ...
    logger.info('rtt : %s', str(rtt))
    logger.info('bdw : %s', str(bdw))
    
    minRttNode = rtt.get(min(rtt.keys()))
    logger.info('Scheduler select feasible node %s: ',str(minRttNode))
